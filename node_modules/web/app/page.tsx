...
export default function HomePage() {
  ...
  return (
    <div className="space-y-xl p-4 md:p-6 max-w-5xl mx-auto">
      {/* Hero */}
      <section className="bg-surface-container-lowest rounded-xl p-6 card-shadow">
        <h1 className="text-headline-lg text-on-surface mb-2">Готовы к ежедневному обучению?</h1>
        ...
        <div className="flex gap-3 flex-wrap">
          <Link
            href="/learn"
            className="flex items-center gap-2 bg-primary text-on-primary px-6 py-3 rounded-lg text-interactive-btn hover:opacity-90 transition-opacity"
          >
            Начать обучение <ArrowRight className="w-5 h-5" />
          </Link>
          <Link
            href="/decks"
            className="flex items-center gap-2 border border-outline px-6 py-3 rounded-lg text-interactive-btn text-on-surface hover:bg-surface-container-low transition-colors"
          >
            Мои колоды
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-surface-container-lowest rounded-xl p-4 card-shadow flex flex-col items-center">
        ...
        <div className="bg-surface-container-lowest rounded-xl p-4 card-shadow flex flex-col items-center justify-center">
      ...
      {decks.length > 0 ? (
        <section>
          <h2 className="text-headline-md text-on-surface mb-4">Активные колоды</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {decks.slice(0, 6).map((deck) => (
              <Link
                key={deck.id}
                href={`/decks/${deck.id}`}
                className="bg-surface-container-lowest rounded-xl p-4 card-shadow hover:card-active transition-shadow block"
              >
        ...
        <section className="text-center py-12 bg-surface-container-lowest rounded-xl card-shadow">
          <Target className="w-12 h-12 text-outline mx-auto mb-4" />
          <div className="text-body-lg text-on-surface-variant mb-4">У вас пока нет колод</div>
          <Link
            href="/decks/new"
            className="inline-flex items-center gap-2 bg-primary text-on-primary px-6 py-3 rounded-lg text-interactive-btn hover:opacity-90 transition-opacity"
          >
